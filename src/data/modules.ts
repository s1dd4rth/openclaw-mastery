    ...
    phases: [
        {
            ...
            steps: [
                ...
                },
                {
                    ...
                    learn:
                        'OpenClaw needs a host machine that stays on. Pick one of three paths — from Module 2 onward the rest of the course is identical regardless of which you choose.\n\n### Path A — Hostinger VPS (cloud, ~$5–10/mo, recommended for most)\n\nA managed cloud VPS with Hostinger\'s one-click OpenClaw template. Public URL out of the box, runs 24/7, every channel works without extra setup.\n\n1. Go to [Hostinger OpenClaw setup](https://levelup-labs.ai/HOSTINGER-OPENCLAW) and create an account\n2. Watch the setup video: [youtu.be/JXWmkPCcF7E](https://youtu.be/JXWmkPCcF7E)\n3. Choose a VPS plan and select the OpenClaw template\n4. Paste your API key (from the previous step) when the setup wizard asks\n5. Wait 1–3 minutes for the VPS to deploy. You\'ll see a web chat URL when it\'s done.\n\n### Path B — Mac mini (your hardware, no monthly fee)\n\nRun OpenClaw on a Mac mini you already own. No monthly cost; you trade that for two reliability caveats covered below.\n\n1. Install OpenClaw via the canonical installer script (requires Node 22.16+, Node 24 recommended):\n   ```\n   curl -fsSL https://openclaw.ai/install.sh | bash\n   ```\n   Full install docs at [docs.openclaw.ai/install](https://docs.openclaw.ai/install) — covers npm/pnpm/bun, Docker, and managed-host options if you prefer one of those over the script.\n2. Run `openclaw onboard` in a terminal and paste your API key when prompted. For background auto-start (recommended on a Mac mini that lives on your desk), add `--install-daemon` to register a LaunchAgent that restarts the gateway on login.\n3. Open `http://localhost:18789` in a browser. The web chat appears when the gateway is ready.\n\n**One known limitation on the Mac mini path** (covered when you reach it):\n\n- **Module 4 (cron jobs):** Mac mini sleeps when idle by default. Scheduled jobs only fire while the machine is awake. Disable sleep in System Settings or use `caffeinate` to keep cron reliable.\n\n(Module 3 / Telegram needs nothing special on a Mac mini: OpenClaw reaches Telegram by *outbound long-polling* — your Claw calls Telegram\'s API; Telegram never connects in to you — so it works from a localhost-only gateway with no public URL or tunnel. Mac mini and Hostinger are equivalent for Telegram.)\n\n### Path C — Windows via WSL2 (your hardware, Windows host)\n\nWindows users run OpenClaw inside WSL2 (Windows Subsystem for Linux). This gives you the same Unix-y environment Linux users have. The native Windows path (cmd / PowerShell) is not supported by OpenClaw\'s skill ecosystem — recipes assume bash + GNU coreutils, neither of which exists outside WSL.\n\n1. **Enable WSL2** if you haven\'t already. Open PowerShell as Administrator and run `wsl --install` — Windows installs Ubuntu by default. Reboot when prompted, then complete the Ubuntu first-run (set username, password).\n2. **Open Ubuntu** (search "Ubuntu" in the Start menu) — this is your Linux shell.\n3. **Install OpenClaw** with the canonical installer:\n   ```\n   curl -fsSL https://openclaw.ai/install.sh | bash\n   ```\n4. **Run `openclaw onboard`** and paste your API key when prompted.\n5. **Open `http://localhost:18789`** in your Windows browser — WSL2 forwards the localhost port to the Windows host automatically. Web chat appears when ready.\n\n**Known limitations on the WSL2 path:**\n\n- **Same as Mac mini for cron** — your Windows machine sleeps; scheduled jobs only fire while it\'s awake. Prevent sleep in Windows power settings. Telegram needs nothing special: OpenClaw uses outbound long-polling, so it works without a public URL. Tailscale (install the Windows Tailscale app, not inside WSL — it\'ll route to your WSL services; Module 1\'s remote-access step covers this) is only for reaching the web Control UI from your other devices — not for Telegram.\n- **WSL filesystem performance** — keep your `~/.openclaw/` inside the WSL filesystem (the default), not on a Windows drive (`/mnt/c/...`), or skill loading and config reads will be 10–100× slower.\n\nOnce your gateway responds (the Hostinger web chat URL, or `http://localhost:18789` for Mac mini / WSL2), continue to the next step.',
                },
                {
                    id: 'install-validator',
                    title: 'Install the Course Validator Skill',
                    learn:
                        'The course uses a companion OpenClaw skill called `openclaw-mastery` to verify your setup at the end of each module. Instead of you ticking checkboxes by hand, the skill inspects your actual OpenClaw state — config values, files, cron jobs, channels — and returns a structured pass/fail report this app reads.\n\nIt\'s read-only. It never modifies your setup, never displays secrets, and never sends messages. Source: [github.com/s1dd4rth/openclaw-mastery-skill](https://github.com/s1dd4rth/openclaw-mastery-skill) (MIT, 50 checks across 10 modules).\n\n### Install — git-clone into your workspace skills directory\n\nThe `openclaw skills install` CLI today only accepts ClawHub slugs. Until this skill is published on ClawHub, install it manually by cloning into your workspace\'s `skills/` dir — OpenClaw auto-discovers any `SKILL.md` it finds there on the next session start.\n\nOpenClaw\'s active workspace is at `~/.openclaw/workspace/` (the singular `workspace/` directory that holds your `SOUL.md`, `USER.md`, `AGENTS.md`, etc.). Skills go in `~/.openclaw/workspace/skills/`.\n\n**Step 1 — create the skills directory if it doesn\'t exist yet** (a fresh OpenClaw install has no workspace skills, so this dir is usually missing). Hostinger users: SSH into your VPS or use the web terminal. Mac mini: open Terminal.\n\n```\nmkdir -p ~/.openclaw/workspace/skills\n```\n\n**Step 2 — clone the skill in.**\n\n```\ngit clone https://github.com/s1dd4rth/openclaw-mastery-skill ~/.openclaw/workspace/skills/openclaw-mastery\n```\n\n**Step 3 — start a fresh OpenClaw session** (or restart the gateway) so it picks up the new skill.\n\n**Step 4 — verify.**\n\n```\nopenclaw skills list\n```\n\n`openclaw-mastery` should appear with source `openclaw-workspace` (workspace-scoped skills take precedence over bundled ones).\n\n### Once it\'s on ClawHub (future, easier)\n\nWhen this skill is published to ClawHub, install becomes one line:\n\n```\nopenclaw skills install openclaw-mastery\n```\n\nUntil then, the git-clone path above is the only working install method.\n\n### If install fails\n\n- **`openclaw skills list` doesn\'t show it.** Confirm the path: `ls ~/.openclaw/workspace/skills/openclaw-mastery/SKILL.md` should print the file. Confirm you started a fresh OpenClaw session after cloning — workspace skills load at session start, not on the fly.\n- **`~/.openclaw/workspace/` doesn\'t exist on your install.** Some OpenClaw versions use `~/.openclaw/agents/<agent-id>/` instead. Check `ls ~/.openclaw/` — whichever sibling holds your `SOUL.md` is the workspace; clone into `<that-dir>/skills/openclaw-mastery`.\n- **The skill shows up but verification still fails per check.** That\'s an alpha-recipe issue (see below), not an install problem.\n\n### Why this step exists\n\nWithout the skill installed, the later "Use openclaw-mastery to verify module N" prompts have nothing to invoke. The skill is the trusted insider that bridges between this web app and your live OpenClaw — it\'s what lets verification be automated without weakening OpenClaw\'s prompt-injection guards.\n\n### Alpha caveat\n\nThe validator is `v0.1.0-alpha.1`. Structure is complete; a few CLI calls inside the recipes are unverified against live OpenClaw installs. If a specific check looks wrong, fall back to manual toggle — known limitation, not a course bug.',
                    do: {
                        prompt:
                            'On the host running my OpenClaw gateway, do these and report back:\n\n1. Run `mkdir -p ~/.openclaw/workspace/skills` to ensure the skills directory exists.\n2. Run `git clone https://github.com/s1dd4rth/openclaw-mastery-skill ~/.openclaw/workspace/skills/openclaw-mastery` to install the skill.\n3. Start a fresh session and run `openclaw skills list` — confirm `openclaw-mastery` appears with source `openclaw-workspace`.\n\nIf `~/.openclaw/workspace/` does not exist on this install, run `ls ~/.openclaw/` first and tell me what sibling directory holds my SOUL.md — that is my workspace dir. Adjust the paths above accordingly.',
                    },
                    verify: {
                        checks: [
                            {
                                ...
                                verifyPrompt:
                                    'Run `openclaw skills list` for this workspace. Respond ONLY with this JSON: {"checks":[{"id":"validator-installed","pass":true,"detail":"openclaw-mastery v0.1.0-alpha.1 installed (source: openclaw-workspace)"}]} — set pass to false if openclaw-mastery is missing from the list.',
                                failHint:
                                    'Skill not in the list. Confirm: (1) `ls ~/.openclaw/workspace/skills/openclaw-mastery/SKILL.md` should print the file path. (2) Did you start a fresh OpenClaw session after cloning? Workspace skills load at session start, not on the fly.',
                                fixPrompt:
                                    'Tell the Claw: "Run `ls ~/.openclaw/workspace/skills/openclaw-mastery/SKILL.md` to confirm the file is in place. If it is, start a fresh session and re-run `openclaw skills list`. If the file is missing, re-run `mkdir -p ~/.openclaw/workspace/skills && git clone https://github.com/s1dd4rth/openclaw-mastery-skill ~/.openclaw/workspace/skills/openclaw-mastery`."',
                            },
                        ],
                    },
                },
                ...
                },
                {
                    ...
                    learn:
                        '**Hostinger users:** your gateway already has a public URL — skip this step and continue to Phase 2.\n\n**Mac mini users:** your gateway is bound to `127.0.0.1:18789` — only your Mac mini itself can reach it. To use your Claw from your phone or laptop while you\'re away from home, expose it over Tailscale. OpenClaw has built-in Tailscale integration, so no separate reverse proxy needed.\n\n### Why Tailscale\n\nTailscale is a mesh VPN. The free tier covers personal use generously (3 users, 100 devices). Every device you sign in becomes part of your *tailnet* and gets a stable hostname like `mac-mini.your-tailnet.ts.net`. Devices in your tailnet reach each other directly through that hostname from anywhere — no port forwarding, no public exposure unless you opt in (Funnel, below).\n\n### Set it up (~10 minutes)\n\n1. **Create a Tailscale account** at [tailscale.com](https://tailscale.com) (sign in with Google, GitHub, or email).\n2. **Install Tailscale on your Mac mini.** The App Store / cask version is all this course needs — it covers Tailscale **Serve** (private, tailnet-only access to the web Control UI). `brew install --cask tailscale` or download from [tailscale.com/download/mac](https://tailscale.com/download/mac).\n\n   You do **not** need Tailscale **Funnel** for anything in this course. A common misconception is that Telegram needs it — it does not (Telegram uses outbound long-polling, covered in Module 3). Funnel exposes your gateway to the open internet, and Module 1\'s security audit correctly flags `gateway.tailscale_funnel` as a **critical** finding. Only consider it if you have a specific need to reach the gateway from a device *not* on your tailnet, and never without a password gate.\n3. **Sign in:** open the Tailscale app, or run `sudo tailscale up` for the CLI. Your Mac mini joins your tailnet.\n4. **Wire OpenClaw to Tailscale Serve.** Three small steps — each is a real gotcha most Mac mini users hit:\n\n   **a. Symlink the tailscale CLI (Apple Silicon only).** OpenClaw hardcodes the path `/usr/local/bin/tailscale`, but Homebrew on Apple Silicon installs to `/opt/homebrew/bin/`. Symlink it once:\n   ```\n   sudo ln -sf /opt/homebrew/bin/tailscale /usr/local/bin/tailscale\n   ```\n   (Intel Macs already have it at the right path. Run `which tailscale` to check which you have.)\n\n   **b. Enable Tailscale Serve on your tailnet (one-time per tailnet).** The fastest way is to run the serve command once and click the enable link if it prompts:\n   ```\n   /usr/local/bin/tailscale serve --bg --yes 18789\n   ```\n   If Serve isn\'t enabled yet, the CLI prints `Serve is not enabled on your tailnet` with an enable URL. Click it → "Enable" in the admin UI → re-run the command, this time it succeeds.\n\n   **c. Set the gateway config to Serve mode + restart.** With a launchd-managed gateway (the `--install-daemon` setup from Path B), the standalone `openclaw gateway --tailscale serve` command conflicts with the running one and silently fails. Patch the persistent config and kick the daemon instead:\n   ```\n   echo \'{"gateway":{"tailscale":{"mode":"serve"}}}\' | openclaw config patch --stdin\n   launchctl kickstart -k "gui/$(id -u)/ai.openclaw.gateway"\n   ```\n   Wait ~20 seconds, then verify with `tailscale serve status`. You should see your gateway URL with a `proxy http://127.0.0.1:18789` line below it.\n\n5. **Install Tailscale on your phone** (App Store / Play Store), sign in with the same account.\n\n6. **Find your Tailscale URL.** Run `tailscale status` on the Mac mini. Each device shows with its full Tailscale hostname. Personal free-tier accounts use the format `<device>.tail<6chars>.ts.net`. The Tailnet display name in the admin UI (often your email) is separate from this DNS suffix; the DNS suffix is what URLs use. Use whatever `tailscale status` reports for your Mac mini.\n\n7. **Allow your Tailscale URL in the Control UI origin list.** OpenClaw\'s dashboard rejects connections from origins not in `gateway.controlUi.allowedOrigins`. By default only `http://127.0.0.1:18789` is allowed; the Tailscale URL is a different origin so it gets blocked with `origin not allowed`. Add it (substitute your URL from step 6):\n   ```\n   echo \'{"gateway":{"controlUi":{"allowedOrigins":["https://YOUR-CLAW.YOUR-TAIL-SUFFIX.ts.net"]}}}\' | openclaw config patch --stdin\n   launchctl kickstart -k "gui/$(id -u)/ai.openclaw.gateway"\n   ```\n   Wait ~5 seconds.\n\n8. **Test from your phone on cellular** (not your home Wi-Fi). Open the URL from step 6 in Safari. The OpenClaw web chat loads. You\'re talking to your Claw remotely without exposing it to the open internet.\n\n### Telegram (Module 3) does NOT need Funnel\n\nA common misconception: Telegram does **not** require a public URL or Tailscale Funnel. OpenClaw talks to Telegram by *outbound long-polling* — your Claw calls out to Telegram\'s API and Telegram never connects in to you — so it works fine from a localhost-bound gateway behind NAT. Leave Tailscale on **Serve** (tailnet-only). Enabling Funnel exposes your gateway to the open internet and Module 1\'s security audit correctly flags `gateway.tailscale_funnel` as a **critical** finding — so it actively breaks the `audit-no-critical` check for zero benefit. If you ever turned Funnel on, set it back: `openclaw config set gateway.tailscale.mode serve` then restart the gateway.\n\nFull reference: [docs.openclaw.ai/gateway/tailscale](https://docs.openclaw.ai/gateway/tailscale).',
                },
            ],
        },
        {
            ...
            steps: [
                {
                    ...
                    do: {
                        prompt:
                            'Verify both Module 5 skills are present and usable. Do not reinstall or rewrite anything. Report PASS or FAIL for: document-summary installed, document-summary ready, quick-note created, quick-note ready. Give the exact test message for each. Remind me to test from a fresh `/new` session. The validator\'s both-skills-work check confirms both are active.',
                    },
                    verify: {
                        checks: [
                            {
                    ...
                    verify: {
                        checks: [
                            ...
                            },
                            {
                                ...
                                failHint:
                                    'Module 6 keeps SMTP out of scope. Ask your Claw to confirm that SMTP_ values are absent from the config.',
                                fixPrompt:
            ...
            steps: [
                {
                    ...
                    do: {
                        prompt:
                            'Turn on Gmail SMTP for `imap-smtp-email` and add Module 8 outbound rules. Do not ask me to run shell commands.\n\nReuse the Module 6 Gmail setup if present. Ask only for missing values (Gmail address + App Password if needed).\n\nConfigure SMTP in `~/.config/imap-smtp-email/.env`:\n- SMTP_HOST=smtp.gmail.com\n- SMTP_PORT=587\n- SMTP_USER=<my Gmail address>\n- SMTP_PASS=<App Password, never echoed>\n- SMTP_FROM=<my Gmail address>\nConfirm config file permissions are 600.\n\nAdd an `Outbound Email Protocols` section to AGENTS.md: every outbound email requires explicit approval; show full draft with To/Subject/Body; match SOUL.md tone unless asked otherwise; attachments need explicit instruction; ask if recipient is uncertain; Module 8 scope is compose-new only (no reply/forward).\n\nReport PASS/FAIL, imap-smtp-email ready status, where SMTP settings were stored (no secrets printed), AGENTS.md update status, and the exact validation prompt. The validator\'s smtp-configured / config-permissions / outbound-rules checks verify each.',
                    },
                    verify: {
                        checks: [
                            {
            ...
            steps: [
                {
                    ...
                    do: {
                        prompt:
                            'Verify the Module 8 outbound email setup. Do not reinstall or rewrite. Report PASS or FAIL for: Gmail SMTP settings in `~/.config/imap-smtp-email/.env`; config permissions owner-only; imap-smtp-email installed + ready; Outbound Email Protocols in AGENTS.md; follow-up-email skill created.\n\nRemind me to test one approved send + one cancelled send, and to type `/new` first. The validator\'s test-email-sent (via IMAP Sent-folder scan) and approval-gate-works checks verify.',
                    },
                    verify: {
                        checks: [
                            {
                        ...
                        checks: [
                            {
                                ...
                                failHint:
                                    'The Google Form link is included in the validator\'s output. Open it in a new tab before submitting.',
                                fixPrompt:
