    ...
    phases: [
        {
            ...
            steps: [
                ...
                },
                {
                    ...
                    learn:
                        'The course uses a companion OpenClaw skill called `openclaw-mastery` to verify your setup at the end of each module. Instead of you ticking checkboxes by hand, the skill inspects your actual OpenClaw state — config values, files, cron jobs, channels — and returns a structured pass/fail report this app reads.\n\nIt\'s read-only. It never modifies your setup, never displays secrets, and never sends messages. Source: [github.com/s1dd4rth/openclaw-mastery-skill](https://github.com/s1dd4rth/openclaw-mastery-skill) (MIT, 50 checks across 10 modules).\n\n### Install — pick one\n\n**Option A (recommended): terminal one-liner.** On your VPS (Hostinger users: SSH into your VPS, or use the Hostinger web terminal) or your Mac mini, run:\n\n```\nopenclaw skills install https://github.com/s1dd4rth/openclaw-mastery-skill\n```\n\nThen list installed skills to confirm:\n\n```\nopenclaw skills list\n```\n\nYou should see `openclaw-mastery` in the output.\n\n**Option B: ask your Claw to install it for you.** Copy the prompt below into your Claw chat. Your Claw runs the install command on its own host and reports back. Use this if you don\'t want to open a terminal.\n\n---\n\n### If install fails\n\n`openclaw skills install <github-url>` is the canonical CLI per OpenClaw\'s docs, but URL-based install hasn\'t been pinned across all OpenClaw versions yet. If your version refuses the GitHub URL:\n\n- Try `openclaw skills install --help` and look for the documented input format.\n- Fall back: `git clone https://github.com/s1dd4rth/openclaw-mastery-skill ~/.openclaw/workspaces/main/skills/openclaw-mastery` (puts the skill directly in the workspace skills directory) and restart the gateway.\n- Check the [skill\'s README](https://github.com/s1dd4rth/openclaw-mastery-skill#install) for current alternatives.\n\nUntil install succeeds, the verify steps in later modules will fall back to manual toggles — nothing else breaks. The course remains usable.\n\n### Why this step exists\n\nWithout the skill installed, the later "Use openclaw-mastery to verify module N" prompts have nothing to invoke. The skill is the trusted insider that bridges between this web app and your live OpenClaw — it\'s what lets verification be automated without weakening OpenClaw\'s prompt-injection guards.\n\n### Alpha caveat\n\nThe validator is `v0.1.0-alpha.1`. Structure is complete; a few CLI calls inside the recipes are unverified against live OpenClaw installs. If a specific check looks wrong, fall back to manual toggle — known limitation, not a course bug.',
                    do: {
                        prompt:
                            'Run this command on the host running my OpenClaw gateway:\n\n  openclaw skills install https://github.com/s1dd4rth/openclaw-mastery-skill\n\nThen run `openclaw skills list` and confirm `openclaw-mastery` appears in the output. If the install command fails or your OpenClaw version does not accept GitHub URLs, report the exact error and try the git-clone fallback: clone the repo into ~/.openclaw/workspaces/main/skills/openclaw-mastery and restart the gateway.',
                    },
                    verify: {
                        checks: [
                            {
                                ...
                                verifyPrompt:
                                    'Run `openclaw skills list` for this workspace. Respond ONLY with this JSON: {"checks":[{"id":"validator-installed","pass":true,"detail":"openclaw-mastery v0.1.0-alpha.1 installed"}]} — set pass to false if openclaw-mastery is missing from the list.',
                                failHint:
